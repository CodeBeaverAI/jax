...
Ordinarily evaluation of `foo(2.00001)` propagates this primal-tangent pair,
producing `10.0000700001` as the result. Because the primal and tangent
components are well separated in scale in this example, we can visually
interpret the result as the primal-tangent pair (10.0, 7.0), ignoring the the
~1e-10 truncation error at the end.

+++

The idea with forward-mode differentiation is to do the same thing but exactly
and explicitly. We'll represent the primal-tangent pair as an actual pair
instead of folding them into a single floating point number. And for each
primitive operation we'll have a rule that describes how to propagate these
primal tangent pairs. Let's work out the rules for our two primitives.

+++

Consider `x + y` where `x = xp + xt * eps` and `y = yp + yt * eps`
